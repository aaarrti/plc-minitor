...
#ifndef _CMPBLKDRVITF_H_
#define _CMPBLKDRVITF_H_
 ...
 *	1. Systems on which more than 10 network adapters should be used by this block driver.
 *  Example:
 *	MaxInterfaces=12
...
#define BLKDRVTCPKEY_INT_ENABLE_SERVER							"EnableServer"
	...
	#if (SYSTARGET_DEVICE_TYPE == 0x1007) /* SYSTARGET_TYPE_COMMUNICATION (GATEWAY) - compare two macros may not work for all compilers */
		#define BLKDRVTCPKEY_INT_ENABLE_SERVER_DEFAULT				0
	#else
 ...
 * </description>
 */
#define BLKDRVTCPKEY_INT_MAX_SERVER_CONNECTIONS					"MaxServerConnections"
...
#define BLKDRVTCPKEY_INT_LISTENPORT								"ListenPort"
	...
	#if (SYSTARGET_DEVICE_TYPE == 0x1007) /* SYSTARGET_TYPE_COMMUNICATION (GATEWAY) - compare two macros may not work for all compilers */
		#define BLKDRVTCPKEY_INT_LISTENPORT_DEFAULT					INT32_C(11743)
	#else
...
#ifdef CPLUSPLUS
class ICmpBlkDrv : public IBase
{
	public:
};
	#ifndef ITF_CmpBlkDrv
		#define ITF_CmpBlkDrv static ICmpBlkDrv *pICmpBlkDrv = NULL;
	#endif
	#define EXTITF_CmpBlkDrv
#else	/*CPLUSPLUS*/
	typedef ICmpBlkDrv_C		ICmpBlkDrv;
	#ifndef ITF_CmpBlkDrv
		#define ITF_CmpBlkDrv
	#endif
	#define EXTITF_CmpBlkDrv
#endif
